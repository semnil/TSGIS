...
if [ $? -lt 2 ] ; then
    ...
    STEAM_LINK=`curl "${SEARCH_URL}" 2>/dev/null | grep "\"link\":" | grep 'http:\/\/store.steampowered.com\/app\/[0-9]\+' | head -n 1 | sed 's/^.*\"link\": //g' | sed 's/,.*//g'`
fi
...
if [ "${DISPLAY_TITLE}" != "" -a $? -ne 0 ] ; then
    ...
    echo -e "${HIST_TITLE}\thttps://${SERVER_NAME}${REQUEST_URI}\tinfo.sh?title=${PARAM_VAL}" >> ${HIST_FILE}
fi
