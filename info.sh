...
GENRE=`echo ${GENRE} | sed 's/カジュアル//g' | sed 's/独立系開発会社//g' | sed 's/早期アクセス//g' | sed 's/MM（MassivelyMultiplayer）//g' | sed 's/アクションRPG/ARPG/g' | sed 's/シミュレーションRPG/SRPG/g' | sed 's/ハックアンドスラッシュ/ハクスラ/g' | sed 's/アクション/ACT/g' | sed 's/シューティング/STG/g' | tr -d '\r' | sed -e 's/,\+/,/g' | sed 's/^,//g' | sed 's/,$//g' | sed 's/,/\//g'`
...
if [ "$META_LINK" = "" ] ; then
    META_TITLE=`echo ${DISPLAY_TITLE} | tr "A-Z" "a-z" | sed -E "s/([:'\?\.&]|-$)//g" | sed 's/[_-]/ /g' | sed -E 's/ +/-/g'`
...
echo "<td>=HYPERLINK($STEAM_LINK;\"$DISPLAY_TITLE\")</td>"
echo "<td>$DATE</td>"
echo "<td>$GENRE</td>"
[ "$META_LINK" != "" ] && echo "<td>=HYPERLINK($META_LINK;\"$METASCORE_STR\")</td>" || echo "<td>$METASCORE_STR</td>"
echo "<td>$REVIEWS</td>"
[ "$DB_LINK" != "" ] && echo "<td>=HYPERLINK($DB_LINK;\"$PRICE_STR\")</td>" || echo "<td>$PRICE_STR</td>"
[ "$DEVELOPER" == "" ] && echo "<td></td>" || echo "<td>$DEVELOPER</td>"
echo "</tr>"
