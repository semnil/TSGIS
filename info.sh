...
GENRE=`echo ${GENRE} | sed 's/カジュアル//g' | sed 's/独立系開発会社//g' | sed 's/早期アクセス//g' | sed 's/MM（MassivelyMultiplayer）//g' | sed 's/アクション/ACT/g' | sed 's/シューティング/STG/g' | tr -d '\r' | sed -e 's/,\+/,/g' | sed 's/^,//g' | sed 's/,$//g' | sed 's/,/\//g'`
...
if [ "$DEVELOPER" != "" ] ; then
    ...
    QUERY=`echo ${DEVELOPER} | sed 's/&amp;/%26/g' | sed 's/ /+/g' | sed 's/\++/+/g'`+`echo ${DISPLAY_TITLE} | sed 's/ /+/g' | sed 's/\++/+/g'`
