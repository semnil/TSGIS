...
if [ $? -lt 2 ] ; then
    ...
    QUERY=`echo ${INPUT_STR} | sed 's/-/ /g' | sed 's/ /+/g' | sed 's/%/%25/g' | sed 's/:/%3A/g' | sed 's/!/\\!/g' | sed 's/%20/+/g'`
...
if [ "${STATUS}" != "200" ] ; then
    ...
    QUERY=`echo ${DISPLAY_TITLE} | tr "A-Z" "a-z" | sed -E 's/[^a-zA-Z0-9!-.:-@¥[-\`{-~ ]+.*//g' | sed 's/_/ /g' | sed -E 's/ +/-/g' | sed -E "s/([:'\?\.&,\/]|-$|-dx$)//g" | sed -E 's/-+/+/g'`
